...
def extractExtraCreditFeatures7(x):
    ...
    #    feature_dict[key + "^2"] = val^2

    feature_dict["const"] = 1
    feature_dict["first word is the"] = (words[0] == "the")
    feature_dict["# words"] = len(words) > 10
    #feature_dict["# english words"] = len([word for word in words_2 if word in en_words])
    feature_dict["# letters"] = len(list(x)) > 30
    feature_dict["# letters"] = (len(list(x)) / float(len(words))) > 4
   ...
   # feature_dict["# 2-letter words"] = len([word for word in words if len(word) is 2])
    #feature_dict["# 7-letter words"] = len([word for word in words if len(word) is 7]) / float(len(words))
    #feature_dict["# 8-letter words"] = len([word for word in words if len(word) is 8]) / float(len(words))
    #feature_dict["# 9-letter words"] = len([word for word in words if len(word) is 9]) / float(len(words))
    feature_dict["# exclamations"] = (x.count('!')) > 2
    feature_dict["# hyphens"] = x.count('-')
   # feature_dict["# question marks"] = (x.count('?'))
    feature_dict["# other punctuation"] = (x.count('.') + x.count(',') + x.count('(') + x.count(')')) > 8
