...
def solveAliasCSP(traits):
	...
	solver = util.BacktrackingSearch(numSolutions=100, verbose=True)
	solver.solve(csp)
...
def addVariables(csp, traits, whichCSP):
	...
	print "Inside addVariables, whichCSP: ", whichCSP
...
def addFactors_buddyScore(csp, traits):
	...
	for i in xrange(n-1):
	    for j in xrange(i+1, n):
			csp.add_binary_factor(aliasVars[i], aliasVars[j], lambda x, y: aliasBuddyScore(x, y))
